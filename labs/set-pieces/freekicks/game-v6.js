...
(() => {
  ...
  const PITCH = { topY: 66, botY: 528, topW: 420, botW: 940, cx: 480 };

  // Push goal "further back": higher + smaller
  const GOAL = { cx: 480, y: 46, w: 220, h: 120, post: 5, mouthPad: 14 };

  // More realistic FK distance
  const KICKER_SPOT = { u: 0.47, v: 0.16 };
  const BALL_SPOT   = { u: 0.52, v: 0.14 };

  const WALL = { v: 0.44, count: 4, spread: 0.22, radius: 0.030 };
  ...
  function onUp(evt){
    ...
    const strength = 0.55 + power * 1.65; // 0.55..2.2
    state.ball.du = dirU * 0.85 * strength;
    state.ball.dv = (0.82 + dirV * 0.70) * strength;
  ...
  function drawWall(){
    ...
    for (const m of men){
      ...
      const depth = (1.05 - m.v*0.6);
      const w = 40*depth, h = 90*depth;
