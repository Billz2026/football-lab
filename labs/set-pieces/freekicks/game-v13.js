...
(() => {
  ...
  function fireMeterShot(){
    const power = clamp(state.meter.power, 0.10, 1);
    const aimU = clamp(state.meter.aimU, 0.30, 0.70);

    // Side aim relative to ball
    const side = clamp((aimU - state.ball.u) / 0.22, -1, 1);

    // Forward speed (towards goal): v decreases, so dv must be negative
    const fwd = 1.55 + power * 3.35;     // 1.55..4.90 (main range)
    state.ball.dv = -fwd;

    // Lateral speed (to hit corners / bend around wall)
    state.ball.du = side * (0.55 + power * 1.05);

    const mode = state.shotMode || "CURL";
    state.ball.mode = mode;

    // Spin model
    state.ball.spin = side * (0.35 + power * 1.30);
    if (mode === "CURL"){ state.ball.spin *= 1.45; }
    if (mode === "DIP"){  state.ball.spin *= 0.80; }

    if (mode === "KNUCKLE"){
      state.ball.spin *= 0.10;
      state.ball.wob = (0.95 + power * 1.55);
    } else {
      state.ball.wob = 0;
    }

    // Height: needs enough lift to clear wall + reach goal
    state.ball.z = 0;
    state.ball.vz = clamp(0.85 + power * 1.05, 0.55, 1.85);
  ...
  function update(dt){
    ...
    if (!state.ball.flying){
      if (state.meter.phase === "POWER"){
        state.meter.t += dt * 1.35;
        const x = (state.meter.t % 2);
        const tri = x < 1 ? x : (2 - x);
        state.meter.power = 0.25 + tri*0.75;
        if (elMeter) elMeter.textContent = `Power ${((0.25 + tri*0.75)*100)|0}%`;
      } else if (state.meter.phase === "AIM"){
        state.meter.t += dt * 1.15;
        const x = (state.meter.t % 2);
        const tri = x < 1 ? x : (2 - x);
        // Aim sweeps across the goal mouth
        state.meter.aimU = 0.34 + tri * 0.32; // 0.34..0.66
