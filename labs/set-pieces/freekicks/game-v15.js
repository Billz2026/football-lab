...
(() => {
  ...
  const PITCH = { topY: 6, botY: 548, topW: 180, botW: 1040, cx: 480 };

  // Push goal "further back": higher + smaller
  const GOAL = { cx: 480, y: 12, w: 140, h: 78, post: 4, mouthPad: 12 };

  // More realistic FK distance
  const KICKER_SPOT = { u: 0.47, v: 0.36 };
  const BALL_SPOT   = { u: 0.52, v: 0.33 };

  const WALL = { v: 0.70, count: 4, spread: 0.22, radius: 0.019 };
  ...
  const state = {
    ...
    keeper: { u: 0.50, targetU: 0.50, diving: false, diveT: 0, reactDelay: 0.24, speed: 1.75, reach: 0.15 },
  ...
  function onUp(evt){
    ...
    const strength = 0.78 + power * 2.55; // 0.78..3.33 (longer FK distance)
