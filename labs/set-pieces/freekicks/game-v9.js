...
(() => {
  ...
  const PITCH = { topY: 22, botY: 528, topW: 260, botW: 980, cx: 480 };

  // Push goal "further back": higher + smaller
  const GOAL = { cx: 480, y: 24, w: 165, h: 90, post: 4, mouthPad: 12 };
  ...
  function update(dt){
    ...
    if (state.ball.v > 0.94) {
      ...
      const save = inGoalMouth() && bp.y > saveBandTop && Math.abs(bp.x - kx) < (GOAL.w * state.keeper.reach);
