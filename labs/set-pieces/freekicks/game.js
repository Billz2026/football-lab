...
(() => {
  ...
  // Start button: hides the intro overlay (and any win/lose overlay if you want to keep playing)
  if (overlayStartBtn){
    const start = (e)=>{ e?.preventDefault?.(); e?.stopPropagation?.(); hideOverlay(); };
    overlayStartBtn.addEventListener("click", start, { capture:true });
    overlayStartBtn.addEventListener("touchend", start, { capture:true, passive:false });
  }
  ...
  function hideOverlay(){
    // Use explicit display toggles (more reliable on some mobile browsers / CSS overrides)
  ...
  function showStartOverlay(){
    ...
    overlayTitle.textContent = "FREE KICKS";
    overlayText.textContent = "Drag from the ball to aim + set power. Release to shoot. After you shoot, swipe left/right to add curve.";
    if (overlayStartBtn) overlayStartBtn.style.display = "inline-flex";
    overlayNextLink.style.display = "none";
    overlayRetryLink.style.display = "none";
  }
  ...
  renderLevels();
  // Show an intro overlay so it's obvious how to start
  showStartOverlay();
  step();
})();