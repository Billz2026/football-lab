// Football Lab • Free Kicks (Guaranteed playable build)
// If you can see the player+ball but can't shoot, it's usually because an overlay is capturing clicks.
// This build removes overlays and ensures the canvas receives pointer events.
...
(() => {
  ...
  const loadImg = (src) => new Promise((resolve, reject) => {
    ...
    i.src = src + (src.includes("?") ? "" : "?v=" + Date.now()); // bust cache for images
  });
  ...
  // --- INPUT (works on desktop + mobile)
  function onDown(evt) {
    if (state.ball.moving) return;
    const p = getPointerPos(evt);
    const b = project(state.ball.u, state.ball.v);

    // generous hit radius so it works even if the ball render/position differs slightly
    const hit = dist(p.x, p.y, b.x, b.y) <= state.ball.r * 4.0;
  ...
  function onUp(evt) {
    ...
    // Base: send it up-field + a bit towards goal (right)
  ...
  window.addEventListener("mouseup", onUp); // important: if mouse released outside canvas
  ...
  function update(dt) {
    ...
    // out of bounds => reset after a short moment
