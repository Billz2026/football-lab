// Football Lab - Free Kicks (starter)
// Aim + power (drag), then add curve (swipe left/right after kick).
// Levels are URL-based so navigation works everywhere.
...
(() => {
  ...
  function loadLevelById(levelId){
    ...
    const L = LEVELS[idx === -1 ? 0 : idx];

    state.levelId = L.id;
    state.levelIndex = idx === -1 ? 0 : idx;
  ...
  window.addEventListener("touchmove", (e)=>{ pointerMove(e); swipeMove(e); }, { passive:false });
  window.addEventListener("touchend", (e)=>{ pointerUp(e); swipeEnd(); }, { passive:false });
