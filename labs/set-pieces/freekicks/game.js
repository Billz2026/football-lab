...
(() => {
  ...
  if (overlayStartBtn){
    const start = (e)=>{ try{ e.preventDefault(); e.stopPropagation(); }catch(_e){} hideOverlay(); };
    overlayStartBtn.addEventListener("click", start, true);
  ...
  function showStartOverlay(){
    // Intro overlay on load (like a "match start")
    overlay.hidden = false;
    overlay.style.display = "flex";
    overlayTitle.textContent = "FREE KICKS";
    overlayText.textContent = "Drag from the ball to aim + set power. Release to shoot. After the shot, swipe left/right to add curve.";
    // Show only Start for the intro
