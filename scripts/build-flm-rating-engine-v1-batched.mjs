...
async function getJson(endpoint, params = {}, retries = 2) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    if ((response.status === 429 || response.status >= 500) && attempt < retries) {
      await new Promise(resolve => setTimeout(resolve, response.status === 429 ? 5000 * (attempt + 1) : 1200 * (attempt + 1)));
      continue;
    }
...
function flattenSchedule(payload) {
  const rows = [];
  for (const stage of payload.data || []) for (const round of stage.rounds || []) for (const fixture of round.fixtures || []) rows.push(fixture);
  return rows;
}
...
async function main() {
  ...
  const scheduleRows = [], fixtureMeta = new Map();
  for (const season of SEASONS) {
    const payload = await getJson(`/schedules/seasons/${season.id}`);
    const all = flattenSchedule(payload), finished = all.filter(f => Number(f.state_id) === 5);
    scheduleRows.push({ ...season, fixturesInSchedule: all.length, finishedFixtures: finished.length });
    for (const fixture of finished) fixtureMeta.set(Number(fixture.id), { season, schedule: fixture });
  }

  const ids = [...fixtureMeta.keys()];
  const batches = chunk(ids, BATCH_SIZE);
  const players = new Map(), observedTypes = new Set(), aliasMatches = {};
  const batchReports = [];
  ...
  for (let i = 0; i < batches.length; i += 1) {
    ...
    try {
      const payload = await getJson(`/fixtures/multi/${batchIds.join(',')}`, { include: 'lineups.details.type;lineups.player;participants' });
      const fixtures = payload.data || [];
      fixturesReturned += fixtures.length;
      batchReports.push({ batch: i + 1, requested: batchIds.length, returned: fixtures.length, success: true });
      for (const fixture of fixtures) {
        const meta = fixtureMeta.get(Number(fixture.id)); if (!meta) continue;
        const { season } = meta;
        ...
        for (const lineup of fixture.lineups || []) {
          ...
          const group = positionGroup(lineup.position_id ?? lineup.player?.position_id), teamId = Number(lineup.team_id || 0), wm = minutes * season.weight;
          p.actualMinutes += minutes; p.weightedMinutes += wm; p.appearances += 1; p.positions[group] += wm;
          if (teamId) { p.teams[teamId] = (p.teams[teamId] || 0) + wm; if (season.label === '2026/2027') p.currentTeams[teamId] = (p.currentTeams[teamId] || 0) + minutes; if (teamNames.has(teamId)) p.teamNames[teamId] = teamNames.get(teamId); }
          p.seasons[season.label] ||= { minutes: 0, weightedMinutes: 0, appearances: 0 }; p.seasons[season.label].minutes += minutes; p.seasons[season.label].weightedMinutes += wm; p.seasons[season.label].appearances += 1;
          for (const [key, value] of Object.entries(values)) if (!['minutesPlayed','rating'].includes(key)) add(p.stats, key, value * season.weight);
          if (Number.isFinite(values.rating)) { p.ratingNum += values.rating * wm; p.ratingDen += wm; }
          players.set(Number(playerId), p);
        }
      }
    } catch (error) {
      batchReports.push({ batch: i + 1, requested: batchIds.length, returned: 0, success: false, status: error.status || null, reason: error.message, rateLimit: error.rateLimit || null });
  ...
  const successfulBatches = batchReports.filter(b => b.success).length;
  const fullFixtureCoverage = fixturesReturned === ids.length;
  const pass = fullFixtureCoverage && successfulBatches === batches.length && Object.values(peerCounts).every(n => n >= 8) && calibration.length >= 40;

  const report = {
    engine: 'FLM Rating Engine v1', engineVersion: '1.1.0', generatedAt: new Date().toISOString(), provider: 'sportmonks',
    ...
    seasons: scheduleRows,
    retrieval: { method: 'Sportmonks GET Fixtures by Multiple IDs', maxIdsPerRequest: BATCH_SIZE, fixtureIdsPlanned: ids.length, fixtureRecordsReturned: fixturesReturned, batchesPlanned: batches.length, successfulBatches, batchReports, fullFixtureCoverage },
    ...
    qualityGate: { pass, requirements: { complete253FixtureCoverage: fullFixtureCoverage, allBatchesSuccessful: successfulBatches === batches.length, atLeastEightPeersPerGroup: Object.values(peerCounts).every(n => n >= 8), atLeastFortyCalibrationPlayers: calibration.length >= 40 }, nextStep: pass ? 'Sanity-check calibration ratings, then add league-strength and separate physical/mental baseline models.' : 'Do not use ratings yet; resolve failed retrieval/coverage gate.' },
  ...
  console.log(`Fixtures: ${fixturesReturned}/${ids.length} via ${successfulBatches}/${batches.length} batches.`);
...
main().catch(async error => {
  ...
  await mkdir(path.dirname(REPORT_PATH), { recursive: true });
  await writeFile(REPORT_PATH, `${JSON.stringify({ engine: 'FLM Rating Engine v1', engineVersion: '1.1.0', generatedAt: new Date().toISOString(), fatalError: { message: error.message, status: error.status || null, rateLimit: error.rateLimit || null }, qualityGate: { pass: false } }, null, 2)}\n`, 'utf8');
  process.exit(1);
});
