...
function updatePenaltyHud() {
  ...
  if (elements.stageNumber) elements.stageNumber.textContent = "PENALTY TRAINING · 12 YDS";
  if (elements.stageName) elements.stageName.textContent = "ONE VS ONE · PENALTY SPOT";
  ...
  if (chip) chip.textContent = "PENALTY TRAINING · UNLIMITED ATTEMPTS";
...
window.addEventListener("footballlab:phasechange", () => {
  ...
  queueMicrotask(updatePenaltyHud);
});
