...
import { resolveShotPhysics } from "./physics.js?v=5";
import { resizeCanvas, drawScene } from "./render-v5.js?v=5";
import { unlockAudio, playResultSound } from "./audio-v5.js?v=5";
...
function takeShot() {
  ...
  const runUpDuration = 365;
  state.animation = {
    id: `${performance.now()}-${state.stage}-${state.misses}`,
    startedAt: performance.now(),
    runUpDuration,
    flightDuration,
    totalDuration: runUpDuration + flightDuration + 150,
    impactPlayed: false
  };
  state.finishedAnimationId = null;
}
...
function finishShot(animationId) {
  ...
  if (shot.outcome === "GOAL") {
    ...
    const strikeBonus = shot.strikeQuality >= 0.90 ? 350 : shot.strikeQuality >= 0.68 ? 175 : 0;
    const rangeBonus = Math.max(0, stageConfig().distanceYards - 20) * 24;
    const points = 1000
      + state.stage * 85
      + rangeBonus
  ...
  setPhase("result");
  const resultDelay = ["POST", "BAR", "WALL", "SAVE"].includes(shot.outcome) ? 1750 : 1550;
  state.resultTimeout = setTimeout(state.misses >= 3 ? endRun : prepareNextShot, state.misses >= 3 ? resultDelay + 100 : resultDelay);
}
...
function updateMeter(delta) {
  ...
  if (state.phase === "curve") {
    ...
    const label = strikeQualityLabel(state.meterValue);
    elements.meterNumber.textContent = `${label} ${Math.round(percentage)}%`;
  }
}
