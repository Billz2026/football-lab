...
function impactRatioForShot(shot) {
  ...
  if (Number.isInteger(shot.impactIndex)) return Math.max(0.12, Math.min(0.98, shot.impactIndex / Math.max(1, shot.path.length - 1)));
...
function takeShot() {
  ...
  const impactDelayMs = runUpDuration + flightDuration * impactRatio;
