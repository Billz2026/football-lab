import { drawScene as drawBaseScene, resizeCanvas } from "./render-v8.js?v=113";
import { WORLD, state, ctx } from "./core-v6.js?v=32.3";
...
function drawDiagnostics() {
  ...
  ctx.textAlign = "left";
  ctx.fillText("V9 SHOT DIAGNOSTICS · D TO HIDE", x + 14, y + 22);
  ...
  const maxChars = 57;
  const first = reason.slice(0, maxChars);
  const second = reason.length > maxChars ? reason.slice(maxChars, maxChars * 2) : "";
  ctx.fillText(first, left, row);
  if (second) ctx.fillText(second, left, row + 13);
  ctx.restore();
}
