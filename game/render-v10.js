import { drawScene as drawBaseScene, resizeCanvas } from "./render-v9.js?v=9";
...
function drawImpactFeedback(time) {
  ...
  if (elapsed < 0 || elapsed > 520) return;
  const t = clamp(elapsed / 520, 0, 1);
  const palette = outcomePalette(presentation.outcome);
  const intensity = Math.sin(Math.PI * Math.min(1, t * 1.25)) * (1 - t * 0.35);
  ...
  gradient.addColorStop(0, palette.secondary.replace(/\.[0-9]+\)/, `${0.28 * intensity})`));
  gradient.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, WORLD.width, WORLD.height);

  if (elapsed > 75) {
    const titleProgress = clamp((elapsed - 75) / 240, 0, 1);
    const scale = 0.84 + titleProgress * 0.16;
    const alpha = Math.min(1, titleProgress * 2.5) * (1 - clamp((elapsed - 370) / 150, 0, 1));
