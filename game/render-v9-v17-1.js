...
const before = 'from "./render-v15-base.js?v=15"';
const after = `from "${new URL("./render-v17-1-base.js?v=171", import.meta.url).href}"`;
if (!source.includes(before)) {
  throw new Error("V17.1 diagnostic patch failed: base renderer import not found");
}
source = source.replace(before, after);
source = source.replace(/from\s+"(\.\/[^"\n]+)"/g, (_, specifier) => {
  return `from "${new URL(specifier, sourceUrl).href}"`;
});
source += "\n//# sourceURL=football-lab-render-v9-v17-1-generated.js\n";
