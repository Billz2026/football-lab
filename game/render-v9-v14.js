import { drawScene as drawBaseScene, resizeCanvas } from "./render-v14-base.js?v=14";
...
function drawDiagnostics() {
  ...
  const width = 362;
  const height = data ? 274 : 58;
  const x = WORLD.width - width - 18;
  const y = 18;

  ctx.save();
  ctx.fillStyle = "rgba(3,8,5,.92)";
  ...
  ctx.textAlign = "left";
  ctx.fillText("V14 MATCHUP DIAGNOSTICS · D TO HIDE", x + 14, y + 22);

  if (!data) {
    ctx.fillStyle = "rgba(236,244,235,.65)";
    ctx.font = "700 10px system-ui";
    ctx.fillText("Take a shot to inspect the matchup.", x + 14, y + 43);
    ctx.restore();
    return;
  }
