...
import { resolveShotPhysics } from "./physics-v8.js?v=8";
import { resizeCanvas, drawScene } from "./render-v8.js?v=8";
...
function updateMeter(delta) {
  ...
  const stagePressure = Math.min(0.18, state.stage * 0.012);
  const phaseBoost = state.phase === "aim" ? 1.12 : state.phase === "curve" ? 1.08 : 1.04;
  state.meterClock += delta * (phaseBoost + stagePressure);
  ...
  if (state.phase === "power") {
    ...
    state.meterValue = (Math.sin(state.meterClock * (2.78 + stage.aimSpeed * 0.2)) + 1) / 2;
  }
