...
test('Liverpool audit installs the current post-window spine and correct positions', () => {
  ...
  assert.ok(wirtz, 'Florian Wirtz should be in the audited Liverpool squad');
  assert.ok(isak, 'Alexander Isak should be in the audited Liverpool squad');
  assert.ok(barcola, 'Bradley Barcola should be in the audited Liverpool squad');
  assert.ok(araujo, 'Ronald Araujo should be in the audited Liverpool squad');
  assert.ok(jacquet, 'Jeremy Jacquet should be in the audited Liverpool squad');
...
test('Chelsea audit reflects post-window core and removes stale ownership through stacked audits', () => {
  ...
  const rogers = atClub(db, CHELSEA_ID, /Rogers/i);
  const martinez = atClub(db, CHELSEA_ID, /Martinez|Martínez/i);
  const estevao = atClub(db, CHELSEA_ID, /Estevao|Estêvão/i);
  assert.ok(palmer, 'Cole Palmer should be in the audited Chelsea squad');
  assert.ok(caicedo, 'Moises Caicedo should be in the audited Chelsea squad');
  assert.ok(rogers, 'Morgan Rogers should be in the audited Chelsea squad even when the source name is abbreviated');
  assert.ok(martinez, 'Emiliano Martinez should be in the audited Chelsea squad');
...
test('Tottenham audit moves current arrivals from stale source clubs', () => {
  ...
  assert.ok(tonali, 'Sandro Tonali should be in the audited Tottenham squad');
  assert.ok(robertson, 'Andy Robertson should be in the audited Tottenham squad');
  assert.ok(savio, 'Savio should be in the audited Tottenham squad');
  assert.ok(marmoush, 'Omar Marmoush should be in the audited Tottenham squad');
  assert.ok(tosin, 'Tosin Adarabioyo should be in the audited Tottenham squad');
...
test('Newcastle audit reflects rebuild and removes major summer exits', () => {
  ...
  const nico = atClub(db, NEWCASTLE_ID, /Gonzalez|González/i);
  const elanga = atClub(db, NEWCASTLE_ID, /Elanga/i);
  const toure = atClub(db, NEWCASTLE_ID, /Toure|Touré/i);
  assert.ok(botman, 'Sven Botman should be in the audited Newcastle squad');
  assert.ok(thiaw, 'Malick Thiaw should be in the audited Newcastle squad');
  assert.ok(dedic, 'Amar Dedic should be in the audited Newcastle squad');
  assert.ok(nico, 'Nico Gonzalez should be in the audited Newcastle squad');
  assert.ok(elanga, 'Anthony Elanga should be in the audited Newcastle squad');
  assert.ok(toure, 'Bazoumana Toure should be in the audited Newcastle squad even when the source name is abbreviated');
