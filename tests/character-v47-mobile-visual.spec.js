...
async function enterClassic(page) {
  await page.goto("/index.html?capture=viktor-v47-mobile", { waitUntil: "networkidle" });
...
test("V47 Viktor reads as the production striker from the real mobile gameplay camera", async ({ page }) => {
  ...
  expect(body?.extras?.football_lab_build).toBe("47.0.0");
  expect(body?.extras?.football_lab_art_revision).toBe("V47");
  expect(body?.extras?.football_lab_visual_archetype).toBe("elite-english-striker");
  ...
  console.log("VIKTOR_V47_MOBILE_DIAGNOSTICS", JSON.stringify(diagnostics));
  expect(diagnostics.frame?.production3D).toBe(true);
  expect(diagnostics.visible?.production3D).toBe(true);

  await page.locator("#gameCanvas").screenshot({ path: "test-results/viktor-v47-mobile-gameplay.png" });
  await page.screenshot({ path: "test-results/viktor-v47-mobile-full-ui.png", fullPage: true });
});
