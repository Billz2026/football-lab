/* Football Lab Manager — squad number quick assign v2
 * Keeps returning players on last season's shirt numbers where possible,
 * leaves current/manual choices untouched, and auto-fills only genuinely
 * unassigned players. Individual numbers remain manually selectable.
 */
(() => {
  'use strict';

  const VERSION = '2.0.0';
  const STYLE_ID = 'flm-squad-number-quickpick-v2-style';
  let dbPromise = null;
  ...
  function preferredPool(player) {
    ...
    const group = String(player?.positionGroup || '').toUpperCase();
    return [...new Set([...(POOLS[role] || []), ...(POOLS[group] || [])].filter(validNumber))];
  }

  function lastSeasonAssignments(c) {
    const history = Array.isArray(c?.squadNumberHistory) ? c.squadNumberHistory : [];
    if (!history.length) return {};
    const currentSeason = String(c?.squadNumberRegistration?.season || '');
    const prior = [...history].reverse().find(entry => entry?.season && entry.season !== currentSeason && entry.assignments && typeof entry.assignments === 'object');
  ...
  function previousNumberFor(playerId, previous) {
    const value = previous?.[playerId];
    return validNumber(value) ? Number(value) : null;
  }

  function nextFallbackNumber(player, used) {
    for (const number of preferredPool(player)) if (!used.has(number)) return number;
    for (let number = 1; number <= 99; number += 1) if (!used.has(number)) return number;
    return null;
  }

  function updateAutoButton(modal, previous) {
    const button = modal.querySelector('[data-number-auto]');
    if (!button) return;
    const pending = allInputs(modal).filter(input => !validNumber(input.value.trim()));
    const returning = pending.filter(input => previousNumberFor(input.dataset.numberPlayer, previous) != null).length;
    if (!pending.length) {
      button.innerHTML = 'ALL NUMBERS ASSIGNED';
      button.disabled = true;
      return;
    }
    button.disabled = false;
    button.innerHTML = returning
      ? `AUTO ASSIGN ${pending.length}<small>KEEP ${returning} RETURNING NUMBER${returning === 1 ? '' : 'S'}</small>`
      : `AUTO ASSIGN ${pending.length}<small>KEEP EXISTING CHOICES</small>`;
  }
  ...
  function autoAssign(modal, db) {
    ...
    // Pass 1: restore every returning player's previous-season number first.
    // Existing values in the form are never touched, so manual choices always win.
    inputs.filter(input => !validNumber(input.value.trim())).forEach(input => {
      const previousNumber = previousNumberFor(input.dataset.numberPlayer, previous);
      if (previousNumber == null || used.has(previousNumber)) return;
      input.value = String(previousNumber);
      used.add(previousNumber);
      input.dispatchEvent(new Event('input', { bubbles:true }));
    });

    // Pass 2: only genuinely new/unassigned players receive sensible free numbers.
    inputs
      ...
      .forEach(({ input, player }) => {
        const number = nextFallbackNumber(player, used);
    ...
    updateAutoButton(modal, previous);
  }
  ...
  function placePicker(picker, input) {
    ...
    const top = roomBelow >= estimatedHeight + 8 ? rect.bottom + 5 : Math.max(10, rect.top - estimatedHeight - 5);
  ...
  function openPicker(modal, input, player) {
    ...
    const previous = lastSeasonAssignments(career());
    const previousNumber = previousNumberFor(player.id, previous);
    const suggestions = [previousNumber, ...preferredPool(player)]
      .filter(validNumber)
      .filter((number, index, array) => array.indexOf(number) === index)
      .filter(number => !used.has(number) || number === current)
      .slice(0, 6);
    ...
    picker.innerHTML = `
      ...
      <div class="flm-number-suggestions"><div class="flm-number-picker-note">${previousNumber != null ? `LAST SEASON: ${previousNumber} · ` : ''}SUGGESTED FOR ${roleKey(player)}</div>${suggestions.map(number => `<button type="button" data-pick-number="${number}" class="${number === current ? 'is-current' : ''}">${number}</button>`).join('')}</div>
      <div class="flm-number-grid">${Array.from({length:99},(_,index) => index + 1).map(number => `<button type="button" data-pick-number="${number}" ${used.has(number) && number !== current ? 'disabled' : ''} class="${number === current ? 'is-current' : ''}" title="${used.has(number) && number !== current ? 'Already assigned' : 'Available'}">${number}</button>`).join('')}</div>`;
  ...
  async function enhanceModal(modal) {
    ...
    allInputs(modal).forEach(input => {
      const player = byId.get(input.dataset.numberPlayer);
      const previousNumber = previousNumberFor(input.dataset.numberPlayer, previous);
      input.title = previousNumber != null
        ? `Click to choose an available squad number. Last season: ${previousNumber}`
        : 'Click to choose an available squad number';
      input.setAttribute('aria-haspopup', 'dialog');
      input.addEventListener('click', () => openPicker(modal, input, player));
      input.addEventListener('input', () => updateAutoButton(modal, previous));
      input.addEventListener('keydown', event => {
        if (event.key === 'Enter' || event.key === ' ') {
          event.preventDefault();
          openPicker(modal, input, player);
        }
      });

      if (previousNumber != null) {
        const row = input.closest('.flm-number-row');
        const nameBox = row?.children?.[1];
        if (nameBox && !nameBox.querySelector('.flm-number-returning')) {
          const note = document.createElement('small');
          note.className = 'flm-number-returning';
          note.textContent = `LAST SEASON #${previousNumber}`;
    ...
    if (focusInput) setTimeout(() => {
      if (modal.isConnected && !document.querySelector('.flm-number-picker')) openPicker(modal, focusInput, byId.get(focusInput.dataset.numberPlayer));
    }, 180);
  }
