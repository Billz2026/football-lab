/* Football Lab Manager — dense player profile enhancement v1
...
(() => {
  ...
  function profileMeta(player, db) {
    ...
    return `<section class="flm-cm-profile-meta" data-flm-cm-profile-meta>
  ...
  function enhanceHeader(root, player, db) {
    ...
    number.querySelector('.flm-cm-number-value').textContent = assigned ? String(Number(player.shirtNumber)) : '';
    number.querySelector('.flm-cm-number-label').textContent = assigned ? 'SQUAD NUMBER' : 'NUMBER UNASSIGNED';

    const heading = title.querySelector('h2');
    if (heading) heading.textContent = fullName(player);
    ...
    const age = ageFor(player, db.metadata?.snapshotDate || '2026-09-04');
    subline.textContent = [club?.name || 'Unattached', player.primaryPosition || player.positionGroup || '—', nationality(player), age != null ? `Age ${age}` : null].filter(Boolean).join(' · ');
  }
  ...
  function enhanceProfileTab(root, player, db) {
    ...
    let meta = panel.querySelector('[data-flm-cm-profile-meta]');
    if (!meta) {
      grid.insertAdjacentHTML('beforebegin', profileMeta(player, db));
      meta = panel.querySelector('[data-flm-cm-profile-meta]');
    }
    // Refresh volatile match status without rebuilding the attribute cards.
    if (meta) {
      const fresh = document.createElement('div');
      fresh.innerHTML = profileMeta(player, db);
      meta.replaceWith(fresh.firstElementChild);
    }
  }
  ...
  function transferRows(player, db) {
    const c = career();
    const rows = (c?.transfers?.completed || []).filter(move => move?.playerId === player.id);
    return rows.map(move => {
      const from = db.clubs?.find(club => club.id === move.fromClubId)?.name || move.fromClubId || 'Unknown club';
      const to = db.clubs?.find(club => club.id === move.toClubId)?.name || move.toClubId || 'Unknown club';
      const fee = Number(move.fee);
      return { from, to, fee:Number.isFinite(fee)?fee:null, round:move.round };
    });
  }
  ...
  function historyMarkup(player, db) {
    ...
    return `<div class="flm-cm-history-shell" data-flm-cm-history="${esc(player.id)}">
      <div class="flm-cm-history-summary">
        ...
        <div class="flm-cm-history-note"><span>Career appearances loaded</span><strong>${esc(imported.reduce((sum,row)=>sum+(Number(row.apps)||0),0) + totals.apps)}</strong><em>Recorded data only</em></div>
        <div class="flm-cm-history-note"><span>Career goals loaded</span><strong>${esc(imported.reduce((sum,row)=>sum+(Number(row.goals)||0),0) + totals.goals)}</strong><em>Recorded data only</em></div>
      </div>
  ...
  function enhanceHistoryTab(root, player, db) {
    const panel = root.querySelector('[data-flm-profile-panel]');
    if (!panel) return;
    const existing = panel.querySelector(`[data-flm-cm-history="${CSS.escape(player.id)}"]`);
    if (existing) return;
