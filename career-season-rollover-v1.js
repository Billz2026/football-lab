...
function panelCopy(validation) {
  if (validation.ok) {
    return {
      eyebrow: 'SEASON ROLLOVER READY',
      title: `${ROLLOVER_SOURCE_SEASON} → ${ROLLOVER_TARGET_SEASON}`,
      body: 'Replace the bottom three with the Championship promoted clubs, reset the table and generate a new 38-match Premier League campaign.',
      button: `START ${ROLLOVER_TARGET_SEASON}`,
      blocked: false
    };
  }
