// Football Lab Manager — appointment intro compatibility v2.
// Restores announcement -> supporter reaction -> press without re-rendering the
// active stage from inside its own MutationObserver callback.

const SAVE_KEY = 'flm-career-save';
const STYLE_ID = 'flm-appointment-intro-compat-v2-style';
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .${INTRO_CLASS} .flm-appt-hero,.${INTRO_CLASS} .flm-fan-score,.${INTRO_CLASS} .flm-appt-copy{border-color:#2d6dbb!important;background:rgba(7,28,56,.94)!important}
...
function visibleStage() {
  ...
  if (!parts.modal?.classList.contains(INTRO_CLASS) || !parts.modal.classList.contains('is-open')) return '';
  return parts.body?.querySelector('[data-appointment-v066]')?.getAttribute('data-appointment-v066') || '';
}
...
function renderAnnouncement(c, state) {
  if (visibleStage() === 'announcement') return true;
...
function renderFans(c, state) {
  if (visibleStage() === 'fans') return true;
  const parts = openIntroFrame();
  if (!parts) return false;
  ...
  parts.body.querySelector('[data-appt-media]')?.addEventListener('click', () => {
    ...
    window.FLMAppointmentMedia?.refresh?.();
  }, { once: true });
  return true;
}
...
function reconcile() {
  ...
  try {
    ...
    else if (visibleStage()) closeIntroFrame();
  } finally {
    reconciling = false;
  }
}
...
['flm:career-opened', 'flm:career-created', 'flm:career-data-refresh'].forEach(name => {
  document.addEventListener(name, queue);
});
...
window.FLMAppointmentIntroCompatV2 = Object.freeze({ version: 2, reconcile: queue });
