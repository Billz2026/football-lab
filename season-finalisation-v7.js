...
import { finaliseEnglishPyramidBackground } from './english-pyramid-world-v7.js';
...
function pyramidResult(career, completedAt) {
  const pyramid = finaliseEnglishPyramidBackground(career, { completedAt });
  return {
    pyramid,
    championship: pyramid?.championship || null,
    leagueOne: pyramid?.leagueOne || null,
    leagueTwo: pyramid?.leagueTwo || null,
    nationalLeague: pyramid?.nationalLeague || null,
    nationalLeagueNorth: pyramid?.nationalLeagueNorth || null,
    nationalLeagueSouth: pyramid?.nationalLeagueSouth || null,
    stepThreeDivisions: pyramid?.stepThreeDivisions || null,
    stepFourDivisions: pyramid?.stepFourDivisions || null,
    nextChampionshipMembership: pyramid?.membership || null,
    nextLeagueOneMembership: pyramid?.leagueOneMembership || null,
    nextLeagueTwoMembership: pyramid?.leagueTwoMembership || null,
    nextNationalLeagueMembership: pyramid?.nationalLeagueMembership || null,
    nextStepTwoMembership: pyramid?.stepThreeMembership || null,
    nextStepThreeMembership: pyramid?.nextStepThreeMembership || null,
    nextStepFourBoundary: pyramid?.stepFiveBoundary || null
  };
}
...
export function finaliseSeason(career, { completedAt = new Date().toISOString() } = {}) {
  ...
  if (ranking.playoffRequired) {
    ...
    return { status: 'playoff-required', outcome: null, resolution: career.seasonResolution, ...lowerWorld };
  }
