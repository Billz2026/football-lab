import { syncCareerNews } from './career-news-v046.js?v=0.4.7';
...
function ensureStyles(){
  ...
  .flm-manager-progress span{height:4px;border-radius:0;background:#1f4674}.flm-manager-progress span.is-active{background:#55dc7c;box-shadow:0 0 12px #55dc7c55}.flm-manager-field span{color:#8fa9bd}.flm-manager-field input,.flm-manager-field select{border-color:#24558e;border-radius:2px;background:#061326;color:#eef3f6}.flm-manager-field input:focus,.flm-manager-field select:focus{border-color:#55dc7c;box-shadow:0 0 0 1px #55dc7c55}.flm-manager-actions button{border-color:#2b63a4;border-radius:2px;background:#0a2242;color:#e9f1f6}.flm-manager-actions button:hover,.flm-manager-actions button:focus-visible{border-color:#55dc7c;color:#55dc7c;outline:none}.flm-manager-actions .primary{border-color:#f4c342;background:#f4c342;color:#071326}.flm-manager-actions .primary:hover,.flm-manager-actions .primary:focus-visible{background:#ffd75e;color:#071326}.flm-exp-card{border-color:#24558e;border-radius:2px;background:#071c38}.flm-exp-card:hover,.flm-exp-card:focus-visible{border-color:#55dc7c;background:#0b315c;outline:none}.flm-exp-card em{color:#55dc7c}.flm-exp-card.is-selected{border-color:#55dc7c;background:#174d36;box-shadow:inset 4px 0 #55dc7c}.flm-manager-summary{border-color:#24558e;border-radius:2px;background:#061326;color:#9caebe}.flm-manager-summary strong{color:#f4c342}.flm-manager-strip-v064>div{border-color:#24558e;background:#071c38}.flm-manager-strip-v064 .identity strong{color:#f4c342}
...
function patchBriefings(c,db){
  ...
  if(Array.isArray(c.news?.items))c.news.items=c.news.items.filter(item=>!['manager-appointed','fan-reaction-appointment'].includes(item.key));const items=c.news?.items||[];
  const board=items.find(i=>i.key==='board-expectation');
  if(board){board.title='The board sets its expectations';board.body=`The board has set a clear target for the season: ${expectation.primary}. They would regard ${expectation.minimum} as acceptable and see ${expectation.stretch} as an outstanding campaign. You have ${formatMoney(budget.transferBudget)} available for transfers and ${formatMoney(budget.wageRoom)} per week in wage room. Progress will be judged by results, performances and the way the squad is managed.`;board.priority='important';}
  const welcome=items.find(i=>i.key==='welcome');if(welcome&&c.managerProfile?.schemaVersion){const club=db.clubs.find(x=>x.id===c.clubId);const contract=c.managerContract||managerContractFor(c,db);welcome.body=`On behalf of the board, welcome to ${club?.name||'the club'}, ${c.managerProfile.name}. We are delighted to have you as our manager and believe you are the right person to lead the team forward. Your contract has been signed until ${fullDate(contract.endDate)} on a salary of ${formatMoney(contract.weeklyWage)} per week. The board looks forward to seeing your plans take shape on the pitch.`;}
  const early=items.find(i=>i.key==='competition-briefing');if(early){const first=openingFixtures(c,db);early.title='Your opening fixtures are ready';early.source='Club Secretary';early.body=first.length?`Your first match is ${first[0].replace(/^1\. /,'')}. The rest of your opening run is ${first.slice(1).join(' · ')}. Use the time before kick-off to assess the squad and settle your approach.`:`Your opening fixtures are being finalised. The complete schedule will appear in Fixtures as soon as it is released.`;early.priority='important';}
